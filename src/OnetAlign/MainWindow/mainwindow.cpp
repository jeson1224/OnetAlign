    ...
    , m_customizeWidget(nullptr)
{
    ...
    cost.start();
	setWindowTitle(("Onet Align System"));


	m_centralForm = QSharedPointer<CentralForm>(new CentralForm());
	setCentralWidget(m_centralForm.get());

	//m_edit = new QTextEdit(this);
	m_status =  new QLabel(this);
	m_status->setText("运行");
	m_statusMsg =  new QLabel(this);
	m_statusBars = new QStatusBar(this);
	m_statusBars->addWidget(m_status);
	m_statusBars->addWidget(m_statusMsg);
	setStatusBar(m_statusBars);
    PRINT_COST(cost, lastTimes, "setCentralWidget & setWindowTitle");
    SARibbonBar *ribbon = ribbonBar();

    //添加方法1
	ribbon->applicationButton()->setText(("Onet"));
    ...
    connect(customize2, &QAction::triggered, this, [&]() {
        ...
        if (SARibbonCustomizeDialog::Accepted == dlg.exec()) {
            ...
            if (isok) {
				...
				m_statusMsg->setText("write xml:");
				m_statusMsg->setText(str);
            }
        }
    });
...
void MainWindow::createCategoryMain(SARibbonCategory *page)
{
    ...
    connect(act, &QAction::triggered, this, [this](bool b) {
		this->m_statusMsg->setText("action clicked");
    });
...
{
	m_statusMsg->setText(QString("MenuButtonPopupCheckableTest : %1").arg(b));
}


void MainWindow::onInstantPopupCheckableTest(bool b)
{
	m_statusMsg->setText(QString("InstantPopupCheckableTest : %1").arg(b));
}


void MainWindow::onDelayedPopupCheckableTest(bool b)
{
	m_statusMsg->setText(QString("DelayedPopupCheckableTest : %1").arg(b));
}


void MainWindow::onMenuButtonPopupCheckabletriggered(bool b)
{
	m_statusMsg->setText(QString("MenuButtonPopupCheckabletriggered : %1").arg(b));
}


void MainWindow::onInstantPopupCheckabletriggered(bool b)
{
	m_statusMsg->setText(QString("InstantPopupCheckabletriggered : %1").arg(b));
}


void MainWindow::onDelayedPopupCheckabletriggered(bool b)
{
	m_statusMsg->setText(QString("DelayedPopupCheckabletriggered : %1").arg(b));
}
